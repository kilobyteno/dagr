...
If MinIO exits with `Fatal glibc error: CPU does not support x86-64-v2`, set these on the resource (Available at Buildtime, so Compose interpolates the image tags):
...
Coolify cannot change the host CPU. On Proxmox you can set the VM CPU to `host` or `x86-64-v2-AES` and leave these unset.
