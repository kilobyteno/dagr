...
Start the app with `make web-dev`. Choose **Self-hosted** and enter `http://localhost:8080` (or the host you published).

See [Sign in](/desktop/sign-in).
...
| --- | --- |
| `PUBLIC_BASE_URL` | Origin used in invite and email verification links. Local default is `http://localhost:5173`. |
