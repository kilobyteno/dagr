...
To put the same services on Northflank instead, see [Northflank](/hosting/self-hosting/northflank).
...
Open `http://localhost:4173` in a browser, or start the Electron app with `make client-dev`. Choose **Self-hosted** and enter `http://localhost:8383` (or the host you published).
...
Do not run `make compose-up` and `make run-watch` together. Both bind port `8383`.

## Check it

```bash
curl -s http://localhost:8383/api/v1/health
# {"status":"ok"}

curl -s http://localhost:8383/api/v1/public/config
```
...
```bash
docker compose --project-directory . -f deploy/docker-compose.yml down -v
```

## Before you expose it

- Change default Postgres and MinIO passwords.
- Put TLS in front of port `8383`.
...
To run this stack behind a panel instead, see [Coolify](/hosting/self-hosting/coolify) or [Northflank](/hosting/self-hosting/northflank).
