...
Both messages are plain text. The link inside uses `PUBLIC_BASE_URL` (invite accept and `/verify-email?token=…`). Set that to the public API origin people can open, not `localhost`, if the server is on the internet. The API serves those paths.
