...
Open `http://localhost:4173`. Choose **Self-hosted** and `http://localhost:8383`.
...
| `VITE_DAGR_SELF_HOSTED_URL` | `http://localhost:8383` | `https://YOUR_API_HOST` |
...
Use the same Compose file as [Coolify](/hosting/self-hosting/coolify) (`/deploy/docker-compose.yml`, base directory `/`).

1. Attach a public domain to **`client`** on port `80`.
2. Attach a public domain to **`dagr`** on port `8383`.
