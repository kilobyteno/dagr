...
| `REDIS_URL` | Redis `REDIS_MASTER_URL` |
...
The image entrypoint is `/dagr`. If you only override the command, migrate will not run. On **CMD override**, choose custom entrypoint and custom command:
...
Use the exact origin from step 6, including `https://` and with no trailing slash. Invite and verification links use `PUBLIC_BASE_URL`.
...
2. On **CMD override**, choose custom entrypoint: `/dagr-worker`.
3. Do **not** add a public port.
4. Create the service and wait until it is running.
