...
4. Check that `Dagr_<version>.dmg` and `Dagr_<version>.exe` are on the GitHub Release. A tag such as `0.1.0-alpha.4` is a prerelease. A stable tag such as `0.1.0` is created as a draft until you publish it.
