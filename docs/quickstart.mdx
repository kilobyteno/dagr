...
This starts Postgres, Redis, MinIO, runs migrations, starts the API on `http://localhost:8383`, and serves the web UI on `http://localhost:4173`.
...
Open `http://localhost:4173` in a browser. On the login screen choose **Self-hosted** and use `http://localhost:8383`. Create an account, then create a workspace. That seeds a `#general` channel.
